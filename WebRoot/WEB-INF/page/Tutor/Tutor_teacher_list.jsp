...
<body class="user-select">
    ...
    <div class="wrap">
        <!-- 查询-->
        <div class="seach">
            <form action="<%=basePath%>Tutor_openList.action" method="post">
        ...
        <table>
            ...
            <s:iterator value="teacherPageResult.data">
                <tr>
                    ...
                    <td class="sysUser-teacherTitle">${teacherTitle }</td>
                    <td>
                    	<button type="button" class="btn btn-info btn-xs" onclick="show_modal(this,'修改教师信息','modify',${sysUserId});">修改</button>
	                    <button type="button" class="btn btn-danger btn-xs" onclick="show_modal(this,'删除教师','del',${sysUserId});">删除</button>
                    </td>
                </tr>
            </s:iterator>
            </tbody>
	...
	<!-- 触发脚本-->
	<script type="text/javascript">
		var modifyMsgURL = '<%=basePath %>Tutor_openEditByAjax.action',
	         ...
	         if(typeAction == "del"){
	         	...
	         	doModifyAjax($('.modal-body'),modifyMsgURL,data,function(backData){
	         		...
	         		var temp = '<div class="input-group mar-t">'
							+'    <span class="input-group-addon">导师账号</span>'
							...
							+'	</div>'
							+'  <div class="input-group mar-t">'
							+'    <span class="input-group-addon">导师密码</span>'
							...
							+'	</div>'
							+'  <div class="input-group mar-t">'
							+'    <span class="input-group-addon">导师职称</span>'
							+'	  <input type="text" class="form-control" value="'+backData.sysUser.teacherTitle+'" name="teacherTitle">'
							+'	</div>'
							+'	<div class="input-group mar-t">'
							+'		<span class="input-group-addon">导师性别</span>'
					...
					$('.modal-body').find('#topicInfo-instituteId').append(xueyuan);
					//绑定修改按钮
					document.getElementById('id_ad_search').onclick = function () {
					/*
					userId
					...
					*/
                        ...
                        var teacherTitle = $('.modal-body').find('input[name="teacherTitle"]').val();//teacherTitle
                        if(userId.length <= 0){
						...
						var data = {
							...
							"sysUser.teacherTitle":teacherTitle
						};
						//timer
						doModifyAjax(obj,doModifySaveURL,data,function(msg){
							...
							if(msg == true || msg == 'true'){
								...
								$(trDom).find('td.sysUser-teacherTitle').html(teacherTitle);
    ...
    <!-- 模态框（Modal） -->
	<script type="text/javascript">
		var url = "<%= basePath%>/Tutor_openList.action";                 //获取表单url
		//首页
		function first(){
